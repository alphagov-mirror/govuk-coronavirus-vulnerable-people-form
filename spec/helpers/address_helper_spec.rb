...
RSpec.describe AddressHelper, type: :helper do
  ...
  describe "#remove_selected_address_changes" do
    it "returns the same address if all fields exist in the Ordance Survery Places API response definition and all values are strings" do
      ...
      expect(helper.remove_selected_address_changes(selected_address)).to eq returned_address
    end

    it "removes the UPRN and all fields where the key does not exist in the Ordance Survery Places API response definition" do
      ...
      expect(helper.remove_selected_address_changes(changed_selected_address)).to eq returned_address
    end

    it "removes the UPRN and all fields where the value is not a string" do
      changed_selected_address = <<~ADDRESS.squish
        {
          "UPRN" : "123456789",
          "SAO_TEXT" : null,
